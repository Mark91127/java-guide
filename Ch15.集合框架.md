...
- 為無序，除非使用有序的介面（如 `TreeMap` 或 `LinkedHashMap`）。
...
最常用的 `Map` 介面的介面類之一，基於`哈希表（HashTable）`資料結構的實現。
...
是`Java`集合框架中的一個介面，主要用於存儲有序且允許重複的元素。提供了操作集合中特定位置元素的方法，使其成為一種基於索引操作的集合類型。

##### 特性

- 有序性: 元素按插入順序存儲，遍歷時保持插入順序。
- 允許重複元素： 可以存儲多個值相等的元素。
- 基於索引操作： 元素可以通過索引（從 `0` 開始）訪問、插入或刪除。

##### 介面類

- `ArrayList` - 基於動態數組的類，用於存儲有序且允許重複的元素。
- `LinkedList` - 基於雙向鏈表的類，用於存儲有序且允許重複的元素。
...
基於動態數組的類，用於存儲有序且允許重複的元素，介面了`List`介面，是最常用的集合類之一。
...
- 適合頻繁隨機訪問、讀取操作，如查找元素、按索引訪問等。
- 非線程安全。

**如何動態擴容：**

觀察`ArrayList`是如何做到動態擴容。
...
經過判斷後發現已超出`capacity`大小，建立新的`array`並給予`1.5`倍的`capacity`，再把原陣列進行資料`copy`。
...
基於雙向鏈表的類，用於存儲有序且允許重複的元素。它介面了`List`和`Deque`介面，支持作為列表和雙端隊列使用。
...
- 是基於`Map`介面。
- 保證集合中的所有元素是唯一的，避免不小心插入重複數據的情況。
- 不保證存儲元素的順序（除非是特定介面，例如`LinkedHashSet`或`TreeSet`）。

##### 介面類

- `HashSet` - 基於`HashMap`介面。
- `TreeSet` - 基於`TreeMap`介面。
