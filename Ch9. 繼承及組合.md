...
是物件導向程式設計的核心特性之一，允許一個類別（子類別）繼承另一個類別（父類別）的屬性和方法，從而實現代碼的重用和結構的清晰性。
...
`Cat`和`Dog`都是`動物`，所以我們可以透過`繼承`達到統一效果，讓`成員變數`或`方法`不用各自定義。
...
雖然透過`繼承`統一了方法，但是`Cat`和`Dog`的行為都會一致嗎? 例如`叫`這個行為，就會有出現問題，所以接下來要介紹
`覆寫（Overriding）`。

### 覆寫（Overriding）

是`Java`繼承中的一個重要概念，允許子類別`重新定義`父類別的`方法`行為。這是實現`多型`的重要手段。

**宣告方式：**

- 當子類別使用與父類別`完全相同`的`方法名稱`，`參數列表`和`返回類型`定義一個方法時，就稱為覆寫（Overriding）。
- 子類別的方法會`取代`父類別的方法。

在未使用`覆寫`的情況下，所以各自定義`叫`的方法：

`Cat`建立`meow()`，因為貓通常是用喵的：
...
但並不是每個`動物`都是這樣`叫`，所以運用`覆寫`達到彈性處理。
...
呼叫當前對象當前的`成員變數`，避免`模糊(ambiguous)`導致邏輯錯誤。
...
`super`關鍵字指的是父類別的引用，用於訪問父類別的方法或屬性。
...
`子類別`的構造函數中可以使用 `super(...)` 呼叫`父類別`的構造函數，確保`父類別`的初始化。
...
如果`父類別`有`有參數建構子`，因為是`父類別`的必要參數，所以`子類別`就必須處理，例如：`動物`有`種類`，所以只要是一個`動物`
就一定會有`種類`。
...
訪問父類別的`成員變數`，避免`模糊(ambiguous)`導致邏輯錯誤。
...
假設我們有`SuperCar`、`NormalCar`等，如果按照繼承概念，都是一個`Car`，所以都會繼承`Car`，如果又區分`XXXSuperCar`、
`XXXNormalCar`，那階層關係就會複雜許多，就會面臨到`為了一片葉子卻獲得整片森林`困境。
...
相較於`繼承`，無需再建立`XXXSuperCar`、`XXXNormalCar`等巢狀繼承，我們只需要`Car`一個類別，透過更換`Engine`就可以達到目的，也只需負責
`Engine`的組件維護。
...
- 繼承：`Car` 是一種 `Vehicle`，表示本質上是一類交通工具。
